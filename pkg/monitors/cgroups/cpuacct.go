	...
	walkControllerHierarchy(controllerPath, func(cgroupName string, files []string) {
		...
		for _, f := range files {
			...
			err = withOpenFile(f, func(fd *os.File) {
				...
				if err != nil {
					logrus.WithError(err).Errorf("Failed to parse %s", f)
					return
				}
				dps = append(dps, usage.Datapoints()...)
			})
			if err != nil {
				logrus.WithError(err).Errorf("Could not process %s", f)
			}
		}

		for i := range dps {
			dps[i].Dimensions["cgroup"] = cgroupName
		}
	})
